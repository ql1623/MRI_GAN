...
def train():
    ...
    # seg_class = options.SEG_CLASS
    # if seg_class == "multiclass":
    #     multiclass = True
    # else:
    #     multiclass = False
    # # num_modalities = 3   
    ...
    # for epoch in range(1):
        ...
        for index, images_labels in enumerate(loop):
            ...
            # else: # if is embedding, only need output mod label to be class
            #     target_labels = get_mod_class_indices(in_out_comb, 3).long()
            ...
            loss_G_reconC = criterion_L1(image_C_recon, image_C)
        ...
        log_loss_to_txt(options.SAVE_CHECKPOINT_DIR, options.SAVE_RESULTS_DIR_NAME, run_id, epoch+1,
                        ...
                        loss_data=[loss_G_BCE, loss_G_L1, loss_G_reconA, loss_G_reconB, loss_G_reconC, loss_G_GDL_ATTN, loss_D_fake, loss_D_real, loss_S_BCE, loss_S_DICE], 
                        loss_name=["G_BCE", "G_L1", "G_reA", "G_reB", "G_reC", "G_GDL_ATTN", "D_fake", "D_real", "S_BCE", "S_DICE"]
                        )
