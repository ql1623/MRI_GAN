...
class FusionBlock_v3_with_cond(nn.Module):
    def __init__(self, in_channels, out_channels, groups, num_classes, img_size, num_mod=2): 
        ...
        self.group_conv =  nn.Sequential(
            nn.Conv2d(in_channels*(num_mod+num_classes), out_channels, kernel_size=3, padding=1, groups=groups),
            nn.InstanceNorm2d(out_channels),
            nn.LeakyReLU(0.2, inplace=True),
        )
    ...
    def forward(self, x1, x2, x3, labels): 
        labels = labels.unsqueeze(2).unsqueeze(3).repeat(1, 1, self.img_size, self.img_size).float()
        condition = self.embed(labels)
        x = torch.cat((x1, x2, x3, condition), dim=1) 
        x = self.group_conv(x)
        return x
...
class datasetGAN(nn.Module):
    ...
    def forward(self, inputs, output_condition):
        ...
        bottleneck_x3, upconv1_x3, upconv2_x3, upconv3_x3, upconv4_x3, out_x3= self.unet2(x3)
