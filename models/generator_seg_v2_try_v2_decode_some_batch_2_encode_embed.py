...
class datasetGAN(nn.Module):
    ...
    def __init__(self, input_channels, output_channels, ngf, pre_out_channels, norm="instance"): # input_channels = 1, output_channel = 1, ngf = 64/32 , features = [16,32,64,128]
        ...
        # self.cond_unet1 = ConditionalBlock_v3(in_channels=1, out_channels=1, feat_img_size=256)
        # self.unet1 = Unet_maxpool(self.input_channels + 1, self.output_channels, self.features)
        # self.cond_unet2 = ConditionalBlock_v3(in_channels=1, out_channels=1, feat_img_size=256)
        # self.unet2 = Unet_maxpool(self.input_channels + 1, self.output_channels, self.features)
        
        # self.cond_unet3 = ConditionalBlock_v3(in_channels=1, out_channels=1, feat_img_size=256)
        # self.unet3 = Unet(self.input_channels + 1, self.output_channels, self.features)
        ...
        # self.fusion0 = FusionBlock_v2(self.features*8, self.features*8, groups=self.features*8, num_mod=2, with_concat_cond=True, is_separate=True,)
    ...
    def forward(self, inputs, input_1_labels, input_2_labels, output_condition):
        ...
        # cond_x1 = self.cond_unet1(input_1_condition)
        # bottleneck_x1, upconv1_x1, upconv2_x1, upconv3_x1, upconv4_x1, out_x1= self.unet1(torch.cat((x1, cond_x1), dim=1))
        # cond_x2 = self.cond_unet2(input_2_condition) 
        # bottleneck_x2, upconv1_x2, upconv2_x2, upconv3_x2, upconv4_x2, out_x2= self.unet2(torch.cat((x2, cond_x2), dim=1))
        # bottleneck_x3, upconv1_x3, upconv2_x3, upconv3_x3, upconv4_x3, out_x3= self.unet3(x3)
        
        # ----- fusion part -----

        # condition_feat = self.cond_v2(output_condition) # [B, C=256, stack=3, feat_img_size, feat_img_size]
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=1, features=[32,64,128,256,512], norm="instance"):
        ...
        self.intitial_conv = nn.Sequential(
            ...
            nn.Conv2d(in_channels + 2, features[0], kernel_size=3, stride=2, padding=1),
...
def test():
    ...
    # x_gen = torch.randn((2,2,256,256))
    # gen = datasetGAN(1,1,32, 16)
    # condition_1 = torch.tensor([0,1])
    # condition_2 = torch.tensor([0,2])
    # condition_out = torch.tensor([2,1])
    # # # print(model_idx.shape)
    # out_fusion, out_x1, out_x2, pre_out_fusion = gen(x_gen, condition_1, condition_2, condition_out)
...
# try_v2_decode, datasetGAN(1,1,32, 16, use_attn=False) with single conv, sampling block, with fusion decoder, instance norm in 
# modality specific and bottleneck that i forgot to add all these time
...
# try_v2_decode, datasetGAN(1,1,32, 16) with single conv, sampling block, with 3 encoder unet and fusion decoder, instance norm in 
# modality specific and bottleneck that i forgot to add all these time
...
# try_v2_decode, datasetGAN(1,1,32, 16) with single conv, maxpool, with 2 encoder unet and fusion decoder, instance norm in 
# modality specific and bottleneck that i forgot to add all these time
###### cond in gen is conv3d instead of conv2d 
# Total params: 11,903,067
# Trainable params: 11,903,067
# Non-trainable params: 0
# Total mult-adds (G): 39.99
# ===============================================================================================
# Input size (MB): 0.50
# Forward/backward pass size (MB): 298.50
# Params size (MB): 45.41
# Estimated Total Size (MB): 344.41

##########################################################################################################################

# if res with need identity each time and maxpool after in unet
# Total params: 19,706,692
# Trainable params: 19,706,692
# Non-trainable params: 0
# Total mult-adds (G): 63.48
# ===============================================================================================
# Input size (MB): 0.75
# Forward/backward pass size (MB): 568.50
# Params size (MB): 75.18
# Estimated Total Size (MB): 644.43

# if res with need identity only first time and sampling after in unet
# Total params: 18,901,083
# Trainable params: 18,901,083
# Non-trainable params: 0
# Total mult-adds (G): 50.80
# ===============================================================================================
# Input size (MB): 0.50
# Forward/backward pass size (MB): 420.50
# Params size (MB): 72.10
# Estimated Total Size (MB): 493.10

# try_v2_decode, datasetGAN(1,1,32, 16) with single conv, maxpool, with 2 encoder unet and fusion decoder,
# instance norm in modality specific and bottleneck that i forgot to add all these time
###### cond in gen is conv3d instead of conv2d 
###### res at fusion decoder
# Total params: 11,907,707
# Trainable params: 11,907,707
# Non-trainable params: 0
# Total mult-adds (G): 40.30
# ===============================================================================================
# Input size (MB): 0.50
# Forward/backward pass size (MB): 314.50
# Params size (MB): 45.42
# Estimated Total Size (MB): 360.42


##########################################################################################################################
# SegmentationNetwork(16, 1)
# Total params: 1,339,777
# Trainable params: 1,339,777
# Non-trainable params: 0
# Total mult-adds (G): 10.07
# ==========================================================================================
# Input size (MB): 4.00
# Forward/backward pass size (MB): 144.50
# Params size (MB): 5.11
# Estimated Total Size (MB): 153.61

# SegmentationNetwork(16, 1) res
# Total params: 1,964,897
# Trainable params: 1,964,897
# Non-trainable params: 0
# Total mult-adds (G): 15.66
# ==========================================================================================
# Input size (MB): 4.00
# Forward/backward pass size (MB): 171.50
# Params size (MB): 7.50
# Estimated Total Size (MB): 183.00

# SegmentationNetwork(32, 1) res
# Total params: 7,936,129 smtg