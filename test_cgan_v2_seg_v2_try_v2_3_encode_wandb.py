...
import model.cgan.generator_seg_v2_try_v2_decode_some_batch_3_encode_embed as models 
from dataset_png_v3_seg_v2_multiclass_seg_3_encode_kfold import MRI_dataset  
...
def test(load_dir_name, input_comb_seed, fold):
    ...
    for index, images_labels in enumerate(test_loader):
        image_A, image_B, image_C, real_target_C, real_seg, in_out_comb, in_out_ohe, img_id = images_labels[0], images_labels[1], images_labels[2], images_labels[3], images_labels[4], images_labels[5], images_labels[6], images_labels[7]
        ...
        with torch.no_grad():
            ...
            pred_target, _, _, _, fusion_features= gen(x_concat, target_labels)
            # # segment
        ...
        # save_results_seg_v4(run_id, img_id, in_out_comb, options.INPUT_MODALITIES, index, image_A_np, image_B_np, pred_target_np, real_target_C_np, pred_seg_mapped, real_seg_mapped, error_metrics, options.SAVE_RESULTS_DIR, options.LOAD_RESULTS_DIR_NAME)
...
if __name__ == "__main__":
    # test()
    test("binary_3_encode_gdl_attn_sobel_f0_wandb", (1,2,0), 0)
    test("binary_3_encode_gdl_attn_sobel_f1_wandb", (1,2,0), 1)
    test("binary_3_encode_gdl_attn_sobel_f2_wandb", (1,2,0), 2)
    test("binary_3_encode_gdl_attn_sobel_f3_wandb", (1,2,0), 3)
    test("binary_3_encode_gdl_attn_sobel_f4_wandb", (1,2,0), 4)